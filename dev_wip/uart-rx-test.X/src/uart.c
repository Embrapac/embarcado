...
// Tempo de acionamento do LED de RX em milissegundos.
#define UART1_RX_LED_ON_MS  1000UL

static volatile uint32_t uart1_rx_led_timer_ms = 0u;
...
/* ===============================================================
 * uart1_rx_poll
 * Verifica se houve recepcao na UART1. Ao detectar qualquer byte,
 * rearma o LED em RB15 por 1 segundo. Os bytes recebidos sao lidos
 * do FIFO para evitar overrun de recepcao.
 * =============================================================== */
void uart1_rx_poll(void)
{
    ...
    {
        rx_data = (uint8_t)U1RXREG;
        (void)rx_data;

        LATBbits.LATB15 = 1u;
        uart1_rx_led_timer_ms = UART1_RX_LED_ON_MS;
    }
}

/* ===============================================================
 * uart1_rx_led_service_1ms
 * Deve ser chamada a cada 1 ms. Mantem o LED de RX em RB15 aceso
 * pelo tempo programado e o apaga ao final da contagem.
 * =============================================================== */
