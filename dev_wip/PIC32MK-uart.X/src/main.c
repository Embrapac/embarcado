/*
 * File:   main.c
 * Author: Matheus Grossi
 *
 * Comportamento:
 *   A cada 10 s envia o byte 0x18 (00011000b) via UART1 (RC0/RC1)
 *   e pisca o LED RB10 durante o envio.
 *
 * Temporizacao do ciclo de 10 s:
 *   blink_led()  = 2000 ms (low) + 2000 ms (high) = 4000 ms
 *   delay_ms()   = 6000 ms
 *   Total        = 10 000 ms
 */

/* 1. Ativa os config bits definidos em defs.h */
#define _CONFIG_BITS_SOURCE

/* Libs do fabricante */
#include <xc.h>
#include <stdint.h>
#include <sys/attribs.h>

/* Libs do projeto */
#include "defs.h"
#include "pin_declaration.h"
#include "aux_func.h"
#include "blink.h"
#include "uart.h"

/* Byte a transmitir: 0x18 = 0b00011000 */
#define DADO_UART   ((uint8_t)0x18u)

/* Tempo total do ciclo em ms */
#define CICLO_MS    10000UL

/* Tempo ocupado pelo blink_led() em ms */
#define BLINK_MS    1000UL

/*=============================================================
 * main
 *=============================================================*/
int main(void)
{
    /* Inicializa pinos de proposito geral */
    pins_init();

    /* Inicializa UART1 (RC0 = TX, RC1 = RX, 115200 8N1) */
    uart1_init();

    while (operation)
    {
        /* Envia o dado */
        uart1_send_byte(DADO_UART);

        /* Pisca o LED para sinalizar envio (4000 ms) */
        blink_led();

        /* Aguarda o restante do ciclo de 10 s */
        delay_ms((uint32_t)(CICLO_MS - BLINK_MS));
    }

    return 0;
}
